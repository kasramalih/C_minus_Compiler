class Codegen:
    def __init__(self):
        ...
        self.st[('output', 0)] = [0, self.current_available_address - 8, self.current_indirect_address - 4, 'void',
                                  None,
                                  None, None, 1, [['int', 504]]]
        self.st[('main', 0)] = [0, self.current_available_address - 16, self.current_indirect_address - 12, 'void', 0,
                                0, 0, 0, [['int', 504]]]
        self.iter_s = []
        self.fst = []
    ...
    def code_gen(self, action, lexeme, line_no):
        ...
        """
        if action == "#pid":
            addr, indirect_addr = self.findaddr((lexeme, self.scope), False)
            self.ss.append('@' + addr)
        elif action == "#d_pid":
            addr, indirect_addr = self.findaddr((lexeme, self.scope), True)
            self.ss.append('@' + addr)
            ...
            if function_lexeme != 'main':
                ...
                self.st[(function_lexeme, 0)] = [self.scope, function_lexeme_address[1:], return_type, self.i,
                                                 return_address,
                                                 return_val, no_args, []]
            else:
                self.fst.append((0, 0))
            ...
            indir_address = None
            self.st[(function_lexeme, 0)][-2] += 1
            for k in self.st.keys():
                if str(self.st[k][1]) == str(address[1:]):
                    indir_address = self.st[k][2]
            self.st[(function_lexeme, 0)][-1].append(['int', address, str(indir_address)])
        elif action == '#param':
            param = self.ss.pop()
            ...
            for k in self.st.keys():
                if str(self.st[k][1]) == str(param[1:]):
                    indir_address = self.st[k][2]
            self.st[(function_lexeme, 0)][-2] += 1
            self.st[(function_lexeme, 0)][-1].append([type, param, indir_address])
        elif action == '#arr_param':
            function_lexeme = self.ss[-1]
            self.st[(function_lexeme, 0)][-1][-1][0] = 'array'
            ...
            if func_name == 'output':
                ...
                if self.st[(func_name, 0)][-1][cnt - 1][0] == 'array':
                    self.add_and_increment_pb(
                        generate_3address_code('ASSIGN', arg[1:], self.st[(func_name, 0)][-1][cnt - 1][1][1:]))
                else:
                    self.add_and_increment_pb(
                        generate_3address_code('ASSIGN', '#' + str(self.st[(func_name, 0)][-1][cnt - 1][2]),
                                               self.st[(func_name, 0)][-1][cnt - 1][1][1:]))
                    self.add_and_increment_pb(
                        generate_3address_code('ASSIGN', arg, self.st[(func_name, 0)][-1][cnt - 1][1]))
            self.ss.append(cnt)
            ...
            self.add_and_increment_pb(
                generate_3address_code('ASSIGN', '#' + str(self.i + 2), self.st[(func_name, 0)][-4]))
            # self.add_and_increment_pb(generate_3address_code('PRINT', '#444444444'))
            ...
            self.add_and_increment_pb(generate_3address_code('JP', self.st[(func_name, 0)][3]))
            if self.st[(func_name, 0)][2] == 'int':
                t_return = str(self.gettemp())
                self.add_and_increment_pb(generate_3address_code('ASSIGN', self.st[(func_name, 0)][-3], t_return))
    ...
    def findaddr(self, inp, declare):
        if declare:
            ...
            lexeme, scope = inp
            if scope == 1:
                if (lexeme, 1) in self.st.keys():
                    return str(self.st[(lexeme, 1)][1]), str(self.st[(lexeme, 1)][2])
                else:
                    return str(self.st[(lexeme, 0)][1]), str(self.st[(lexeme, 0)][2])
            else:
                return str(self.st[(lexeme, 0)][1]), str(self.st[(lexeme, 0)][2])
